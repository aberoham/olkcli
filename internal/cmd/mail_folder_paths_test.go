...
func TestMailMoveResolvesDelegatedFolderPathAndMovesInTargetMailbox(t *testing.T) {
	requests := 0
	_, calls, err := runMailCommand(
		t,
		[]string{"mail", "move"},
		[]string{"--json", "--mailbox", "shared@example.com", "message-id", "Inbox/2026"},
		func(req *http.Request) *http.Response {
			requests++
			switch requests {
			case 1:
			...
			case 2:
				...
				return graphJSONResponse(req, `{"value":[{"id":"year-id","displayName":"2026","childFolderCount":0}]}`)
			case 3:
				if got := req.URL.Path; got != "/v1.0/users/shared@example.com/messages/message-id/move" {
					t.Errorf("move request path = %q", got)
				}
				var payload struct {
					DestinationID string `json:"destinationId"`
				}
				...
				if payload.DestinationID != "year-id" {
					t.Errorf("destination ID = %q, want year-id", payload.DestinationID)
				}
				return graphJSONResponse(req, `{"id":"moved-id"}`)
			default:
				t.Fatalf("unexpected Graph request: %s", req.URL)
	...
	if err != nil {
		t.Fatalf("delegated mail move: %v", err)
	}
	if calls != 3 {
		t.Fatalf("Graph requests = %d, want 3", calls)
	}
...
func TestMailMoveInvalidMailboxAndDelegatedDryRun(t *testing.T) {
	for _, tc := range []struct {
		mailbox   string
		wantError bool
	}{{"not-an-address", true}, {"shared@example.com", false}} {
		t.Run(tc.mailbox, func(t *testing.T) {
			output, calls, err := runMailCommand(t, []string{"mail", "move"}, []string{"message-id", "Inbox/2026", "--mailbox", tc.mailbox, "--dry-run"}, func(req *http.Request) *http.Response {
			...
			if !tc.wantError && !strings.Contains(output, "shared@example.com") {
				t.Fatalf("preview omits target: %s", output)
			}
		})
	}
}
